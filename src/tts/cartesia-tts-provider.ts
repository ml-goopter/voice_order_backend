...
/**
 * Odoo `res.lang` code (`en_US`) → Cartesia ISO-639-1 language (`en`): take the primary subtag and
 * lowercase it. Returns `undefined` for an absent/blank code so the caller can fall back to a default.
 */
export function toCartesiaLanguage(code: LangCode | undefined): string | undefined {
 ...
 * into a single complete audio buffer (a standalone mp3). The customer's detected language is mapped to
 * Cartesia's `language` param so a multi-locale voice speaks the reply in the same language the LLM
 * produced it in; when the turn carries no language, `defaultLanguage` is used. A cancel aborts the
...
export class CartesiaTtsProvider implements TtsProvider {
  ...
  async synthesize(text: string, signal: AbortSignal, language?: LangCode): Promise<Buffer> {
    const lang = toCartesiaLanguage(language) ?? this.defaultLanguage;
    const bytes = await this.speak(text, lang, signal);
