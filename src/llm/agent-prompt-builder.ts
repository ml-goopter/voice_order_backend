 ...
 * by simply REPLYING with a spoken message (no tool call) — that reply serves as both a
 * clarifying question and a recommendation (one merged "reply" outcome). The operation contract is
 * unchanged from the old parser prompt.
 */
export function buildAgentSystemPrompt(): string {
  return [
    ...
    '   - Otherwise, REPLY with a short spoken message and DO NOT call any tool. Use a plain reply',
    '     to ask a clarifying question when the request is ambiguous, or to recommend items when',
    '     the customer asked what to get. Your reply is spoken to the customer and ends the turn.',
    'Never both propose and reply in the same turn. Always end with either a propose_cart call or a',
    'spoken reply — never an empty message.',
    '',
    ...
    'The customer utterance may not be in English; phrase clarifications and recommendations in the same language.',
  ].join('\n');
}

/** The user turn: the utterance plus the context the agent reasons over. Candidates are omitted —
 *  the agent retrieves them itself via `search_menu_semantic`. */
