...
export function buildPrompt(input: OrderGraphInput): LlmPrompt {
  const system = [
    ...
    'conversation_history holds prior turns (oldest → newest) and may be used to infer the customer\'s intended operation, resolve references, and understand follow-up requests. However, current_cart remains the sole source of truth for what is currently in the order. Do not blindly replay prior requests; infer only the operation implied by the current customer_text and use current_cart to determine valid line_id values and current item state.',
