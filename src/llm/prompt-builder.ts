...
export function buildPrompt(input: OrderGraphInput): LlmPrompt {
  const system = [
    ...
    'When a `clarification` object is present, you previously asked its `question` and the customer_text IS the `answer` — combine them to resolve the original request. If the utterance plainly does not answer that question, ignore the clarification and treat customer_text as a new request.',
